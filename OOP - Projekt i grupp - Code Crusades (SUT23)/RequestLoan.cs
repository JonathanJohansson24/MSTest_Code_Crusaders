...
namespace OOP___Projekt_i_grupp___Code_Crusades__SUT23_
{
    public class RequestLoan
    {
        ...
        // förenklad version av att räkna ut maxlån
        public static decimal TestCalculateMaxLoan(decimal totalCapital)
        {
            return totalCapital * 5;
        }

        public static decimal TestCalculateInterest(decimal loanAmount) //Testmetod för interestraten
        {
            return loanAmount * InterestRate;
        }
        public static bool TestTryGetLoanAmount(string input, decimal maxLoan, out decimal loanAmount) // Testar ifall man kan ta ett lån
        {
            if (decimal.TryParse(input, out loanAmount) && loanAmount > 0)
            {
        ...
        public static void TestProcessLoan(decimal loanAmount)
        {
            decimal interest = TestCalculateInterest(loanAmount);
            Console.Clear();
            Console.WriteLine($"\n\tRäntesatsen ligger på {InterestRate * 100}%. Det innebär att räntan för lånet är {interest}");
            Loan newLoan = new Loan(loanAmount, InterestRate);
            UserContext.CurrentUser.AddLoan(newLoan);
            DepositLoan(loanAmount);
        }
        public static void TestRequestLoan()
        {
            decimal totalCapital = UserContext.CurrentUser.Accounts.Sum(account => account.Balance);
            decimal maxLoan = TestCalculateMaxLoan(totalCapital);
            ...
            while (true)
            {
                ...
                if (TestTryGetLoanAmount(input, maxLoan, out decimal loanAmount))
                {
                    TestProcessLoan(loanAmount);
                    break;
                }
                else
                {
                    Console.WriteLine("\n\tFel inmatning eller belopp över maxgränsen, försök igen.");
                }
            }
        }
        public static void Loan()
        {
            ...
            while (true)
            {
                if (decimal.TryParse(Console.ReadLine(), out decimal loanInput) && loanInput > 0)
                {
                    ...
                    {
                        decimal interest = TestCalculateInterest(loanInput);
                        Console.Clear();
                        Console.WriteLine($"\n\tRäntesatsen ligger på {InterestRate * 100}%. Det innebär att räntan för lånet är {interest}");                        
